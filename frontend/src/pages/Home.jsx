            ...
            <div className="flex overflow-x-auto no-scrollbar p-2 gap-2">
               {tabs.map(tab => (
                  <button
                     ...
                     className={`flex items-center gap-2 px-4 py-2.5 rounded-xl whitespace-nowrap transition-all text-sm font-bold ${activeTab === tab.id ? 'bg-blue-600 text-white' : 'text-slate-400 bg-slate-700/30'
                        }`}
                  >
               ...
               <nav className="flex flex-col gap-2">
                  {tabs.map(tab => (
                     <button
                        key={tab.id}
                        onClick={() => { setActiveTab(tab.id); setActiveTemplate(null); }}
                        className={`flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 font-bold ${activeTab === tab.id
