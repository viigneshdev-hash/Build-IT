...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (data.user) {
        // Successful signup, redirect to home or show confirmation
        navigate('/home');
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-900 text-white selection:bg-blue-500/30 font-sans">
      ...
      <div className="hidden md:flex flex-col flex-1 bg-gradient-to-br from-blue-900 to-indigo-900 p-16 justify-between items-start relative overflow-hidden group">
        <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-blue-500/10 rounded-full blur-[120px] -z-10 animate-pulse transition-transform group-hover:scale-110"></div>
        ...
        <Link to="/" className="flex items-center gap-3 group/logo">
          ...
          <span className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-white to-blue-200 tracking-tighter">
            Build IT
          </span>
        </Link>
        ...
        <div className="max-w-md space-y-6">
          ...
          <p className="text-xl text-blue-200/70 font-medium">
            Join thousands of professionals who have landed their dream jobs using our AI-driven resume builder.
          </p>
        </div>

        <div className="p-8 bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 w-full max-w-sm group">
          <div className="flex -space-x-3 mb-4">
             {[1,2,3,4].map(i => (
               <div key={i} className="w-10 h-10 rounded-full bg-blue-400/20 border-2 border-slate-900 flex items-center justify-center text-xs font-bold text-white overflow-hidden backdrop-blur-md">
                 <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${i}`} alt="user" />
               </div>
             ))}
             <div className="w-10 h-10 rounded-full bg-blue-500 border-2 border-slate-900 flex items-center justify-center text-[10px] font-black">+2k</div>
          </div>
          <p className="text-sm font-semibold text-blue-100 italic">"This tool doubled my interview calls in just a week!"</p>
          <p className="text-xs text-blue-300/50 mt-1">— Sarah J., Senior Software Engineer</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 sm:p-12 lg:p-24 bg-slate-900 relative">
        ...
        >
          <div className="text-center space-y-3">
            ...
            <p className="text-slate-400 font-medium">Get started with your free resume today.</p>
            {error && (
              <motion.div 
          ...
          <p className="text-center text-slate-400 font-medium">
            Already have an account? <Link to="/login" className="text-blue-400 hover:text-blue-300 font-bold decoration-2 underline-offset-4 hover:underline transition-all">Log in</Link>
          </p>
        </motion.div>
      </div>
    </div>
